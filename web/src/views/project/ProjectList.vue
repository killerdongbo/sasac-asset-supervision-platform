<template>
  <div class="project-list">
    <el-card class="search-card" shadow="never">
      <el-form :model="queryForm" inline>
        <el-form-item label="关键词">
          <el-input v-model="queryForm.keyword" placeholder="项目名称/编号" clearable style="width: 200px" />
        </el-form-item>
    ...
    <div class="toolbar">
      <el-button type="primary" @click="router.push('/projects/create')">新增项目</el-button>
    </div>
    ...
    <el-table :data="projects" v-loading="loading" border stripe style="width: 100%">
      ...
      <el-table-column prop="projectType" label="类型" width="100">
        <template #default="{ row }">
          {{ typeMap[row.projectType] || row.projectType }}
        </template>
      </el-table-column>
      <el-table-column prop="budgetTotal" label="预算总额" width="130" align="right">
        <template #default="{ row }">
          {{ row.budgetTotal ? '¥' + Number(row.budgetTotal).toLocaleString() : '-' }}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="progressPct" label="进度" width="160">
        <template #default="{ row }">
          <el-progress :percentage="row.progressPct || 0" :stroke-width="16" />
        </template>
      </el-table-column>
...
import type { PmProject } from '@/api/project'
...
async function fetchData() {
  ...
  try {
    ...
    const response = await queryProjects(params as any)
