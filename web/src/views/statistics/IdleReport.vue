    ...
    <el-card shadow="never">
      <template #header>
        <div class="header-row">
          <span class="card-title">闲置资产统计</span>
          <el-button type="primary" @click="fetchData">刷新</el-button>
        </div>
      </template>
      <el-row :gutter="20" class="stat-row">
        <el-col :span="8">
          <el-statistic title="闲置资产总数" :value="totalCount" />
        </el-col>
        <el-col :span="8">
          <el-statistic title="闲置原值(万元)" :value="+(totalOriginal / 10000).toFixed(2)" />
        </el-col>
        <el-col :span="8">
      ...
      <el-table :data="tableData" v-loading="loading" stripe style="margin-top: 20px;">
        ...
        <el-table-column label="闲置数量" prop="assetCount" width="120" sortable />
        <el-table-column label="闲置原值(万元)" width="160" sortable>
...
import { getIdleStats } from '@/api/reportStats'
...
function renderChart() {
  ...
  chart.setOption({
    ...
    series: [{ type: 'bar', data: tableData.value.map(d => d.assetCount), itemStyle: { color: '#e6a23c' } }]
  })
}
