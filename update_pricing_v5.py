...
for mc in original_merges:
    ws.unmerge_cells(str(mc))
print(f"Unmerged {len(original_merges)} ranges")

# Step 2: 找到投资管理行并插入7行
invest_row = None
for r in range(1, ws.max_row + 1):
    v = ws.cell(r, 2).value
    if v and '投资管理' in str(v):
        invest_row = r
        break
...
for r, c, v in data:
    ...
    is_title = (c == 1 and v and "二期升级" in str(v))
    cell.font = Font(name='微软雅黑', size=11 if is_title else 10, bold=is_title,
                     color='1F4E79' if is_title else '000000')
    if is_title:
        cell.fill = blue

# Step 4: 重新合并标题行（跳过 Phase 2 区域的新行）
title_merges = []
for mc in original_merges:
    min_col, min_row, max_col, max_row = mc.min_col, mc.min_row, mc.max_col, mc.max_row
    # 在 invest_row+1 之后插入7行，后面的行全部下移7行
    if min_row > invest_row:
        min_row += 7
        max_row += 7
    title_merges.append((min_row, min_col, max_row, max_col))

for min_row, min_col, max_row, max_col in title_merges:
    try:
        ws.merge_cells(min_row=min_row, min_col=min_col, max_row=max_row, max_col=max_col)
...
checks = ['产权管理', '财务监督', '业绩考核', '监督追责', '重大事项', '预警驾驶舱', '硬件', '信创']
all_ok = True
for k in checks:
    found = False
